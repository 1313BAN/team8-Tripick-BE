<%@ page contentType="text/html; charset=UTF-8" language="java" %>
<div class="container-fluid">
  <div class="row align-items-center justify-content-between">
    <!-- 왼쪽 이미지 -->
    <div class="col-md-3 text-start">
      <img
        src="./assets/img/footer_ssafy-logo-black.b301994e.png"
        alt="Company Logo"
        class="footer-logo"
      />
    </div>

    <!-- 오른쪽 가로 4개 이미지 -->
    <div class="col-md-6 d-flex justify-content-end gap-3">
      <img
        src="./assets/img/footer_ssafy-logo-small.bd603755.png"
        alt="Image 1"
        class="footer-img"
      />
      <img
        src="./assets/img/footer_mcst-logo.86700cdd.png"
        alt="Image 2"
        class="footer-img"
      />
      <img
        src="./assets/img/footer_tour-api-logo.ebcb1140.png"
        alt="Image 3"
        class="footer-img"
      />
      <img
        src="./assets/img/footer_visit-korea-logo.858325ef.png"
        alt="Image 4"
        class="footer-img"
      />
    </div>
  </div>

  <!-- 회사 정보 -->
  <div class="text-center mt-4 footer-info">
    <p>
      <strong>대표 :</strong> HI SSAM &nbsp; | &nbsp;
      <strong>주소 :</strong> 212, Teheran-ro, Gangnam-gu, Seoul, Republic of
      Korea &nbsp; | &nbsp; <strong>대표번호 :</strong> 1588-3357 &nbsp; |
      &nbsp; <strong>사업자번호 :</strong> 123-4567-987
    </p>
  </div>
</div>
